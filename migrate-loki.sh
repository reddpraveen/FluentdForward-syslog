...
echo "Starting Loki bucket migration process..."

# Step 1: Create new RGW bucket for Loki
echo "Step 1: Creating new RGW bucket for Loki..."
cat <<EOF | oc apply -f -
...
oc apply -f loki-migration-job.yaml

# Step 4: Monitor the migration progress
echo "Step 4: Monitoring migration progress..."
echo "You can check the progress with: oc logs -f job/loki-bucket-migration -n openshift-logging"

# Wait for job completion
while true; do
    STATUS=$(oc get job loki-bucket-migration -n openshift-logging -o jsonpath='{.status.conditions[?(@.type=="Complete")].status}')
    if [ "$STATUS" == "True" ]; then
        echo "Migration job completed successfully"
        break
    fi
    FAILED=$(oc get job loki-bucket-migration -n openshift-logging -o jsonpath='{.status.conditions[?(@.type=="Failed")].status}')
    if [ "$FAILED" == "True" ]; then
...
spec:
  ...
  - name: target-secret
    secret:
      secretName: loki-rgw-bucket
  restartPolicy: Never
EOF

# Wait for verification pod to complete
while true; do
    STATUS=$(oc get pod loki-verify -n openshift-logging -o jsonpath='{.status.phase}')
    ...
    if [ "$STATUS" == "Failed" ]; then
        ...
        oc logs loki-verify -n openshift-logging
        oc delete pod loki-verify -n openshift-logging
        exit 1
    fi
    sleep 10
done
